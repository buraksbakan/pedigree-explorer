...
from PyQt6.QtWidgets import (
    ...
    QColorDialog, QStatusBar, QInputDialog, QTabWidget
)
...
# ═══════════════════════════════════════════════════════════════════════════
#  IDEOGRAM VIEW  –  pyideogram / matplotlib (view-only)
# ═══════════════════════════════════════════════════════════════════════════
...
class IdeogramPlot(QWidget):
    ...
    def __init__(self):
        ...
        self.all_regions = []       # same structure as ChromosomePlot.all_regions
        self.headers = []
        self.colour_by_column = None
        self.active_annotation_filter = None
        self.annotation_colors = {}
    ...
    def set_colour_state(self, colour_by_column, active_filter):
        """Sync colour state from MainWindow dropdowns."""
        self.colour_by_column = colour_by_column
        self.active_annotation_filter = active_filter
        # Reset colour assignments so they rebuild
        self.annotation_colors = {}
        self.color_index = 0
        self.update_plot()
    ...
    def _draw_overview(self):
        ...
        legend_entries = {}  # annotation_value -> color

        for i, chrom in enumerate(chroms):
            ax = self.figure.add_subplot(gs[0, i])

            # Draw ideogram backbone with centromere
            if chrom in _CHROM_LENGTHS_BP:
                pyideogram.ideogramv(chrom, ax=ax, color=_PLAIN_BAND_COLORS)
            else:
                # Unknown chrom — draw simple rectangle
                clen = 50_000_000
                ax.add_patch(Rectangle((-0.4, 0), 0.8, clen,
                             facecolor=(0.92, 0.92, 0.92), edgecolor="black"))

            # Force shared y-axis for proper scaling
            ...
            for chrom_dict, headers in self.all_regions:
                ...
                for region in chrom_dict[chrom]:
                    ...
                    if annotation_value is not None:
                        ...
                        if self.active_annotation_filter is None:
                            color = self._get_color_for_annotation(annotation_value)
                        elif self.active_annotation_filter == "ALL":
                            color = self._get_color_for_annotation(annotation_value)
                        elif annotation_value == self.active_annotation_filter:
                            color = self._get_color_for_annotation(annotation_value)
                        else:
                            color = "#D5D5D5"  # greyed out

                        if color != "#D5D5D5":
                            legend_entries[annotation_value] = color

                    ax.add_patch(Rectangle(
                        (-0.48, start), 0.96, end - start,
        ...
        if legend_entries:
            ...
            ax_leg.legend(handles=patches, loc="upper left", fontsize=7,
                         framealpha=0.9, title="Annotations", title_fontsize=8)
...
class MainWindow(QMainWindow):
    ...
    def create_layout(self):
        ...
        # Right Side: Tabbed view (Plotly + Ideogram)

            # Tab widget to switch between Plotly and Ideogram views
        self.plot_tabs = QTabWidget()

            # Tab 1: Plotly view (existing)
        plotly_container = QWidget()
        plotly_layout = QVBoxLayout()
        plotly_layout.addWidget(self.chrom_plot)
        plotly_container.setLayout(plotly_layout)
        self.plot_tabs.addTab(plotly_container, "Plotly View")

            # Tab 2: Ideogram view (new)
        ideogram_container = QWidget()
        ideogram_layout = QVBoxLayout()
        ideogram_layout.addWidget(self.ideogram_plot)
        ideogram_container.setLayout(ideogram_layout)
        self.plot_tabs.addTab(ideogram_container, "Ideogram View")
    ...
    def annotation_selection_changed(self, text):
        ...
        # sync ideogram colours
        self.ideogram_plot.set_colour_state(
            self.chrom_plot.colour_by_column,
            self.chrom_plot.active_annotation_filter
        )
