...
class RegionPopup(QDialog):
    ...
    def popupLayout(self):
        ...
        self.show_genes_checkbox = QCheckBox("Show gene markers")
        self.show_genes_checkbox.setChecked(True)
        self.show_genes_checkbox.stateChanged.connect(self.update_plot)
        ...
        self.layout.addLayout(right_side, stretch=3)
    ...
    def update_plot(self):
        ...
        if self.show_genes_checkbox.isChecked():
            for y, gname, gstart, gend in gene_positions:
                ...
                # Label (shifted right + slightly above)
                fig.add_annotation(
                    x=0.85,
                    y=y,   # small vertical offset
                    text=gname,
                    showarrow=False,
