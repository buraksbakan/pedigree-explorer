...
from PyQt6.QtWidgets import (
    ...
    QColorDialog, QStatusBar, QInputDialog, QDialog
)
...
data = pkgutil.get_data("IBD_Viewer", "resources/genes/chr1.bed")
...
class RegionPopup(QDialog):
    def __init__(self, chrom, start, end, genes, parent=None):
...
class MainWindow(QMainWindow):
    ...
    def open_region_popup(self, chrom, start, end):
        ...
        # Create popup
        popup = RegionPopup(chrom, start, end, genes, self)
        popup.exec()
    ...
    def load_gene_annotations(self):
        ...
        for chrom in chroms:
            path = f"resources/genes/{chrom}.bed"
            raw = pkgutil.get_data(__package__, path)
