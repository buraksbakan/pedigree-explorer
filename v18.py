...
with resources.as_file(resources.files("pyideogram.annotation") / "cytoBands.pickle.xz") as f:
    REAL_BANDS = pickle.load(lzma.open(f))
GREY_BANDS = {}

for chrom, (ends, names, types) in REAL_BANDS.items():
    new_types = []
    for t in types:
        if t == "acen":
            new_types.append("acen")   # keep centromere
        else:
            new_types.append("gneg")   # grey for everything else
    GREY_BANDS[chrom] = [ends, names, new_types]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.color_by_dropdown = QComboBox()
        self.color_by_dropdown.addItem("No comparisons")
        self.color_by_dropdown.currentTextChanged.connect(self.color_by_changed)
    ...
    def color_by_changed(self, text):
        #print("color BY CHANGED JUST RAN")
        if text == "No comparisons":
            self.ideogram_plot.color_by_column = None
        else:
            self.ideogram_plot.color_by_column = text
    ...
    def update_color_by_dropdown(self):
        #print("UPDATE color BY DROPDOWN JUST RAN")
        self.color_by_dropdown.blockSignals(True)
        self.color_by_dropdown.clear()
        self.color_by_dropdown.addItem("No comparisons")
        for h in sorted(self.all_headers):
            self.color_by_dropdown.addItem(h)
        self.color_by_dropdown.blockSignals(False)
